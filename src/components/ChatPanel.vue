...
import type { StudyMetricUserRequestSource } from '@/app/study/metrics/types'
...
const outputSurveyPendingRequest = lencanvasOutputQualitySurvey.pendingRequest
const outputSurveyPending = computed(() => outputSurveyPendingRequest.value !== null)
const outputSurveySubmitting = lencanvasOutputQualitySurvey.submitting
const outputSurveyErrorKorean = lencanvasOutputQualitySurvey.submitErrorKorean

async function handleOutputSurveySubmit(values: OutputQualitySurveyAnswerValues): Promise<void> {
  await lencanvasOutputQualitySurvey.submit(values)
}
...
type ChatPanelInsertionValue = { type: 'reasoning-review'; review: ReasoningReview }
...
async function handleSubmit(text: string, requestSource: StudyMetricUserRequestSource = 'new-request') {
  ...
  // A finished hands-off run must be rated before the next request goes out;
  // the input is swapped for the survey card, this guard covers the rest
  // (Continue button, programmatic submits).
  if (outputSurveyPending.value) return
  if (isRunning.value) {
    logStudyMetricEvent({ type: 'mid-run-message', text })
  ...
  logStudyMetricEvent({ type: 'user-request', source: requestSource, text })
  ...
  <div data-test-id="chat-panel" class="flex min-w-0 flex-1 flex-col overflow-hidden select-text">
    ...
    <template v-else>
          ...
          <div v-else data-test-id="chat-messages" class="flex flex-col gap-3">
            ...
            <div v-if="showContinue" class="flex justify-center py-2">
              <button
                ...
                @click="handleSubmit('Continue where you left off', 'continue-run')"
              >
      ...
      <OutputQualitySurveyCard
        ...
        @submit="handleOutputSurveySubmit"
      />
