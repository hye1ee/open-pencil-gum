/**
 * Ported from src/app/user-model/prompt.ts. The domain pack: everything the
...
export function reviseUserPrompt(candidate, neighbours) {
  const existing =
    neighbours.length === 0
...
function userModelSelectionText(selection) {
  ...
  switch (selection.type) {
    ...
    default:
      throw new Error(`Unknown feedback selection: ${String(selection)}`)
  }
}
...
export function feedbackUserPrompt(batch, propositions) {
  const held =
...
export function rationaleUserPrompt(batch, changed, model) {
