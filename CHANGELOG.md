...
- Add experimental Interactive Feedback Notes that compare each completed AI reasoning step with the user model, pause before its action, choose exactly one text, deterministic code visual, or generated image representation, aggregate every reviewed note before proceeding, and retry corrected steps once without another interruption.
